...
int solve(FILE *fptr, int size){
    ...
    char do_chars[] = "do()";
    char dont_chars[] = "don't()";
    int do_index_buffer = 0;
    int dont_index_buffer = 0;
    bool enabled = true;
    ...
    if (fptr != NULL){
        ...
        while (fgets(line, sizeof(line), fptr)){
            ...
            for (int i = 0; i < strlen(line); i++){
                if (line[i] == do_chars[do_index_buffer]){
                    do_index_buffer += 1;
                }
                else{
                    do_index_buffer = 0;
                }
                if (do_index_buffer == 4){
                    printf("\n%sENABLED\n", KGRN);
                    do_index_buffer = 0;
                    enabled = true;
                }
                if (line[i] == dont_chars[dont_index_buffer]){
                    dont_index_buffer += 1;
                }
                else{
                    dont_index_buffer = 0;
                }
                if (dont_index_buffer == 7){
                    dont_index_buffer = 0;
                    enabled = false;
                    printf("\n%sDISABLED\n", KRED);
                }
